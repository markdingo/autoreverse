...
// This function is virtually impossible to test within the Go test framework as a single
// successful test means no others can possibly run as we've thrown away all our
// rights. All we can do is test a few of the error paths and you'll have to have faith
// that the successful code paths have been manually tested...
func TestConstrain(t *testing.T) {
	if os.Getuid() != 0 {
		t.Log("Warning: Cannot test osutil.Constrain() as we're not running as root")
	}
